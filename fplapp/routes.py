...
def home():
    ...
    if type(data[0]) is dict:
        ...
        return render_template('home.html', data = json.dumps(data), league_info= data, chip_dict=fpl_data.get_chip_count(), chip_used_list = fpl_data.get_chips_used_list(),
                                ...
                                count_gw_lowest=fpl_data.count_gw_lowest(), fpl_players=fpl_players.get_team_player_list(), sorted_value=sorted(data, key=lambda k: k['total_value'][-1], reverse=True))
    else:
